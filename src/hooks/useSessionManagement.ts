...
// Глобальное состояние инициализации для предотвращения множественных загрузок
...
export const useSessionManagement = () => {
  ...
  // Оптимизированная загрузка сессий с глобальной инициализацией
  const loadSessions = useCallback(async () => {
    if (initializationState.isInitializing && initializationState.promise) {
    ...
    if (initializationState.isInitialized) {
      try {
        ...
        setSessions(data);
        console.log('Sessions reloaded:', data.length);
      } catch (error) {
        console.error('Failed to reload sessions:', error);
        addError('Ошибка перезагрузки сессий', error instanceof Error ? error.message : 'Unknown error');
      }
      return;
    }
    ...
    const loadPromise = (async () => {
      try {
        ...
        console.log('Sessions initialized:', data.length);
      } catch (error) {
        console.error('Failed to initialize sessions:', error);
        addError('Ошибка инициализации сессий', error instanceof Error ? error.message : 'Unknown error');
        setSessions([]);
      } finally {
        setIsLoading(false);
    ...
    initializationState.promise = loadPromise;
    await loadPromise;
  }, [addError]);
  ...
  // Обновление списка свечей (иммутабельно)
